...
def unique_houses(filename):
    ...
    file_list = open("cohort_data.txt") #output = list of tuples
   
    house_names = []

    for item in file_list:
        person_info = item.split('|')
        if person_info[2] != "":
        # print(person_info)
            house_names.append(person_info[2])

    houses = set(house_names)
...
def sort_by_cohort(filename):
    ...
    file_list = open(filename) # object

    for item in file_list:
        person_info = item.rstrip('\n').split('|') # list of people with their info

        person_name = ' '.join(person_info[0:2]) # output = str of first and last name

        if person_info[-1] == "Winter 2016":
            winter_16.append(person_name)
        elif person_info[-1] == "Spring 2016":
            spring_16.append(person_name)
        elif person_info[-1] == "Summer 2016":
            summer_16.append(person_name)
        elif person_info[-1] == "Fall 2015":
            fall_15.append(person_name)
        elif person_info[-1] == "G":
...
def hogwarts_by_house(filename):
    ...
    file_list = open(filename) # type = object
    ...
    for item in file_list:
        person_info = item.rstrip('\n').split('|') # list of people with their info
        last_name = person_info[1] # last name

        if person_info[2] == "Gryffindor":
            gryffindor.append(last_name)
        elif person_info[2] == "Hufflepuff":
            hufflepuff.append(last_name)
        elif person_info[2] == "Ravenclaw":
            ravenclaw.append(last_name)
        elif person_info[2] == "Slytherin":
            slytherin.append(last_name)
        elif person_info[2] == "Dumbledore's Army":
            dumbledores_army.append(last_name)
        elif person_info[-1] == "G":
            ghosts.append(last_name)
        elif person_info[-1] == "I":
            instructors.append(last_name)


    all_hogwarts = [dumbledores_army] + [gryffindor] + [hufflepuff] + [ravenclaw] + [slytherin] + [ghosts] + [instructors]

    for house in all_hogwarts:
        house.sort()

    # print(all_hogwarts)
    

    return all_hogwarts # list of last names inside list of houses  
